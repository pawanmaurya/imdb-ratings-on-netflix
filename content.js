...
function addIMDBRating(imdbMetaData, name, year) {
  console.log("imdbMetaData",imdbMetaData);
    ...
    if (synopsises.length) {
      ...
      div.innerHTML = 'IMDb rating : ' + (imdbRatingPresent ? imdbRating : "N/A")+ (imdbVoteCount ? ", Vote Count : "+imdbVoteCount:"");
...
function makeRequestAndAddRating(name, year) {
    ...
    xhr.onload = function() {
      if (xhr.status === 200) {
        ...
        var imdbMetaData = imdbRating+":"+imdbVoteCount;
